...
## 서비스와 클라이언트 만들기
서비스에서 서비스를 request 하는 것을 client 노드라 하고, respond 하는 것을 서비스 또는, 서버 노드라고 한다. 요청과 응답의 구조는 `.srv` 파일에 의해 결정된다. 이번 포스팅에서는 클라이언트가 2개의 정수를 넘겨주면 서버에서 2 정수의 합을 응답하는 예제를 만들어 볼 것이다.
...
요청할 때 정수 a,b 를 넘겨주면 응답으로 정수를 돌려주는 것이다.
