...
ROS2에서는 'workspace'라는 개념이 등장한다. Workspace란 ROS2를 사용하고 있는 시스템상의 위치를 의미하는데 일반적으로 동시에 여러개의 workspace가 활성화 된다. (하나의 노드를 workspace라고 이해하면 되지 않을까 싶다.) 이런 workspace 기능이 있기 때문에 하나의 컴퓨터에서 각 workspace마다 다른 버전의 ROS를 사용하여 integrate하는 방식으로 개발이 가능하다.
